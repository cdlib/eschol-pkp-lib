...
class PersonStringNlmNameSchemaFilter extends NlmPersonStringFilter {
	...
	function &_parsePersonsString($personsString, $title, $degrees) {
		// Check for 'et al'.
		$personsStringBeforeEtal = OjsString::strlen($personsString);
		$personsString = OjsString::regexp_replace('/et ?al$/', '', $personsString);
		$etAl = ($personsStringBeforeEtal == OjsString::strlen($personsString) ? false : true);

		// Remove punctuation.
		$personsString = trim($personsString, ':;, ');

		// Cut the authors string into pieces.
		$personStrings = OjsString::iterativeExplode(array(':', ';'), $personsString);
		...
		if (count($personStrings) == 1) {
			...
			foreach($complexPersonsPatterns as $complexPersonsPattern) {
				...
				if ($matched = OjsString::regexp_match($complexPersonsPattern[0], $personsString)) {
					// Retrieve names.
					$success = OjsString::regexp_match_all($complexPersonsPattern[1], $personsString, $personStrings);
	...
	function &_parsePersonString($personString, $title, $degrees) {
		...
		$results = array();
		if ($title && OjsString::regexp_match_get('/^('.$personRegex['title'].')/i', $personString, $results)) {
			$suffixString = trim($results[1], ',:; ');
			$personString = OjsString::regexp_replace('/^('.$personRegex['title'].')/i', '', $personString);
		}

		if ($degrees && OjsString::regexp_match_get('/('.$personRegex['degrees'].')$/i', $personString, $results)) {
			$degreesArray = explode(',', trim($results[1], ','));
			foreach($degreesArray as $key => $degree) {
				$degreesArray[$key] = OjsString::trimPunctuation($degree);
			}
			$suffixString .= ' - '.implode('; ', $degreesArray);
			$personString = OjsString::regexp_replace('/('.$personRegex['degrees'].')$/i', '', $personString);
		}
		...
		$personString = OjsString::regexp_replace('/([A-Z])\.([A-Z][a-z])/', '\1. \2', $personString);
		...
		foreach ($personExpressions as $expressionId => $personExpression) {
			if ($nameFound = OjsString::regexp_match_get($personExpression, $personString, $results)) {
				// Given names
				...
				if (!empty($results['initials'])) {
					...
					for ($initialNum = 0; $initialNum < OjsString::strlen($results['initials']); $initialNum++) {
