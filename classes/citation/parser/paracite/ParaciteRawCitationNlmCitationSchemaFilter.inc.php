...
class ParaciteRawCitationNlmCitationSchemaFilter extends NlmCitationSchemaFilter {
	...
	function &process($citationString) {
		...
		$citationString = OjsString::utf8_to_ascii($citationString);
		...
		if ( Config::getVar('i18n', 'charset_normalization') == 'On' && !OjsString::utf8_compliant($xmlResult) ) {
			$xmlResult = OjsString::utf8_normalize($xmlResult);
		}
		...
		if (isset($metadata['authors'])) {
			$metadata['authors'] = OjsString::trimPunctuation($metadata['authors']);
			$metadata['authors'] = OjsString::iterativeExplode(array(':', ';'), $metadata['authors']);
		}
		...
		foreach(array('title', 'chapter', 'publication') as $titleProperty) {
			if (isset($metadata[$titleProperty])) $metadata[$titleProperty] = OjsString::titleCase($metadata[$titleProperty]);
		}
		...
		foreach ($metadata as $paraciteElementName => $paraciteValue) {
			if (!empty($paraciteValue)) {
				// Trim punctuation
				if (is_string($paraciteValue)) $paraciteValue = OjsString::trimPunctuation($paraciteValue);
		...
		if (isset($metadata['rest_text'])) {
			$nlmDescription->addStatement('comment', OjsString::trimPunctuation($metadata['rest_text']));
		}
