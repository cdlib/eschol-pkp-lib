...
class Citation extends DataObject {
	...
	function _cleanCitationString($citationString) {
		...
		if (Config::getVar('i18n', 'charset_normalization') && !OjsString::utf8_compliant($citationString)) {
			$citationString = OjsString::utf8_normalize($citationString);
		}
		// 2) Strip slashes and whitespace
		$citationString = trim(stripslashes($citationString));

		// 3) Normalize whitespace
		$citationString = OjsString::regexp_replace('/[\s]+/', ' ', $citationString);
