	 ...
	 */
	 function editCitations(&$request, &$submission) {
		...
		if ($citations->getCount() > 0) {
			$initialHelpMessage = \OjsLocale::translate('submission.citations.editor.details.pleaseClickOnCitationToStartEditing');
		} else {
			$articleMetadataUrl = $router->url($request, null, null, 'viewMetadata', $submission->getId());
			$initialHelpMessage = \OjsLocale::translate('submission.citations.editor.pleaseImportCitationsFirst', array('articleMetadataUrl' => $articleMetadataUrl));
		}
