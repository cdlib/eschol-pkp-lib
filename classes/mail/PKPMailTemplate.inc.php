...
class PKPMailTemplate extends Mail {
	...
	function __construct($emailKey = null, $locale = null, $enableAttachments = null, $includeSignature = true) {
		...
		$this->locale = isset($locale) ? $locale : \OjsLocale::getLocale();
	...
	function &processAddresses($currentList, &$newAddresses) {
		foreach ($newAddresses as $newAddress) {
			...
			if (OjsString::regexp_match_get('/^([^<>' . "\n" . ']*[^<> ' . "\n" . '])[ ]*<(?P<email>' . PCRE_EMAIL_ADDRESS . ')>$/i', $newAddress, $regs)) {
				$currentList[] = array('name' => $regs[1], 'email' => $regs['email']);

			} elseif (OjsString::regexp_match_get('/^<?(?P<email>' . PCRE_EMAIL_ADDRESS . ')>?$/i', $newAddress, $regs)) {
