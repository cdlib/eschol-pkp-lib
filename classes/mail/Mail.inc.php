...
class Mail extends DataObject {
	...
	function addAttachment($filePath, $fileName = '', $contentType = '', $contentDisposition = 'attachment') {
		...
		if (empty($contentType)) {
			$contentType = OjsString::mime_content_type($filePath);
	...
	function send() {
		...
		$subject = OjsString::encode_mime_header($this->getSubject());
		$body = $this->getBody();

		// FIXME Some *nix mailers won't work with CRLFs
		if (Core::isWindows()) {
			// Convert LFs to CRLFs for Windows
			$body = OjsString::regexp_replace("/([^\r]|^)\n/", "\$1\r\n", $body);
		} else {
			// Convert CRLFs to LFs for *nix
			$body = OjsString::regexp_replace("/\r\n/", "\n", $body);
		}
		...
		if (Config::getVar('email', 'smtp')) {
			...
			$sent = OjsString::mail($recipients, $subject, $mailBody, $headers, $additionalParameters);
		}
