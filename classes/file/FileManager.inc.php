...
class FileManager {
	...
	function getUploadedFileType($fileName) {
		if (isset($_FILES[$fileName])) {
			$type = OjsString::mime_content_type($_FILES[$fileName]['tmp_name']);
	...
	function downloadFile($filePath, $type = null, $inline = false) {
		...
		if (is_readable($filePath)) {
			if ($type == null) {
				$type = OjsString::mime_content_type($filePath);
	...
	function truncateFileName($fileName, $length = 127) {
		if (OjsString::strlen($fileName) <= $length) return $fileName;
		$ext = FileManager::getExtension($fileName);
		$truncated = OjsString::substr($fileName, 0, $length - 1 - OjsString::strlen($ext)) . '.' . $ext;
		return OjsString::substr($truncated, 0, $length);
	}
