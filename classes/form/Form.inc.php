...
class Form {
	...
	function __construct($template = null, $callHooks = true, $requiredLocale = null, $supportedLocales = null) {
		...
		if ($requiredLocale === null) $requiredLocale = \OjsLocale::getPrimaryLocale();
		$this->requiredLocale = $requiredLocale;
		if ($supportedLocales === null) $supportedLocales = \OjsLocale::getSupportedFormLocales();
	...
	function getDefaultFormLocale() {
		if (empty($formLocale)) $formLocale = \OjsLocale::getLocale();
	...
	function smartyFieldLabel($params, &$smarty) {
		...
		if (isset($params) && !empty($params)) {
			if (isset($params['key'])) {
				$params['label'] = \OjsLocale::translate($params['key'], $params);
			}
