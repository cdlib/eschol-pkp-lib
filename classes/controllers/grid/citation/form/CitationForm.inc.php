...
class CitationForm extends Form {
			...
			foreach($metadataSchema->getProperties() as $propertyName => $property) {
				...
				if (empty($fieldValue)) {
					...
					if ($property->getMandatory()) {
						...
						$this->addError($fieldName, \OjsLocale::translate($property->getValidationMessage()));
	...
	function fetch($request, $template = CITATION_FORM_FULL_TEMPLATE, $display = \false) {
		...
		if ($citation->getId()) {
			...
			$citationDiff = OjsString::diff($this->getData('rawCitation'), $generatedCitation);
		...
		if ($template == CITATION_FORM_FULL_TEMPLATE) {
			...
			foreach($this->_citationProperties as $fieldName => $property) {
				...
				$availableFields[$fieldName] = array(
					'displayName' => \OjsLocale::translate($property->getDisplayName()),
