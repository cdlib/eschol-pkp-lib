...
class UserGroupDAO extends DAO {
	...
	function installSettings($pressId, $filename) {
		...
		foreach ($tree->getChildren() as $setting) {
			...
			$this->installLocale(\OjsLocale::getLocale(), $pressId);
		}
	}
	...
	function installLocale($locale, $pressId = null) {
		...
		while ( !$userGroups->eof() ) {
			...
			$this->updateSetting($userGroup->getId(),
								'name',
								array($locale => \OjsLocale::translate($nameKey, null, $locale)),
								'string',
								$locale,
								true);
			...
			$this->updateSetting($userGroup->getId(),
								'abbrev',
								array($locale => \OjsLocale::translate($abbrevKey, null, $locale)),
								'string',
								$locale,
								true);
			unset($userGroup);
