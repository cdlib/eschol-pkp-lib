...
class HelpTopicDAO extends XMLDAO {
	...
	function &getTopicsByKeyword($keyword) {
		$keyword = OjsString::strtolower($keyword);
	...
	function scanTopic(&$mappingFile, &$matchingTopics,$keyword,$dir,$file) {
		if (preg_match('/^\d{6,6}\.xml$/', $file)) {
			...
			if ($topic) {
				$numMatches = OjsString::substr_count(OjsString::strtolower($topic->getTitle()), $keyword);

				foreach ($topic->getSections() as $section) {
					$numMatches += OjsString::substr_count(OjsString::strtolower($section->getTitle()), $keyword);
					$numMatches += OjsString::substr_count(OjsString::strtolower($section->getContent()), $keyword);
				}
