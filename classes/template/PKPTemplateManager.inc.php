...
define('CDN_JQUERY_UI_VERSION', '1.11.1');
...
class PKPTemplateManager extends Smarty {
	...
	function smartyTruncate($string, $length = 80, $etc = '...', $break_words = false, $middle = false, $skip_tags = true) {
		...
		if (OjsString::strlen($string) > $length) {
			$originalLength = OjsString::strlen($string);
			if ($skip_tags) {
				if ($middle) {
					$tagsReverse = array();
			...
			$length -= min($length, OjsString::strlen($etc));
			if (!$middle) {
				if(!$break_words) {
					$string = OjsString::regexp_replace('/\s+?(\S+)?$/', '', OjsString::substr($string, 0, $length+1));
				} else $string = OjsString::substr($string, 0, $length+1);
				if ($skip_tags) $string = $this->_reinsertTags($string, $tags);
				return $this->_closeTags($string) . $etc;
			} else {
				$firstHalf = OjsString::substr($string, 0, $length/2);
				$secondHalf = OjsString::substr($string, -$length/2);
				...
				if($break_words) {
					...
					for($i=$length/2; $string[$i] != ' '; $i++) {
						$firstHalf = OjsString::substr($string, 0, $i+1);
					}
					for($i=$length/2; OjsString::substr($string, -$i, 1) != ' '; $i++) {
						$secondHalf = OjsString::substr($string, -$i-1);
					}
	...
	function _removeTagsAux($string, $loc, &$tags, $length) {
		if(strlen($string) > 0 && $length > 0) {
			$length--;
			if(OjsString::substr($string, 0, 1) == '<') {
				$closeBrack = OjsString::strpos($string, '>')+1;
				if($closeBrack) {
					$tags[] = array(OjsString::substr($string, 0, $closeBrack), $loc);
					return $this->_removeTagsAux(OjsString::substr($string, $closeBrack), $loc+$closeBrack, $tags, $length);
				}
			}
			return OjsString::substr($string, 0, 1) . $this->_removeTagsAux(OjsString::substr($string, 1), $loc+1, $tags, $length);
		}
	}
	...
	function _removeTagsAuxReverse($string, $loc, &$tags, $length) {
		$backLoc = OjsString::strlen($string)-1;
		if($backLoc >= 0 && $length > 0) {
			$length--;
			if(OjsString::substr($string, $backLoc, 1) == '>') {
				$tag = '>';
				$openBrack = 1;
				while (OjsString::substr($string, $backLoc-$openBrack, 1) != '<') {
					$tag = OjsString::substr($string, $backLoc-$openBrack, 1) . $tag;
					$openBrack++;
				}
				$tag = '<' . $tag;
				$openBrack++;
				...
				$tags[] = array($tag, $loc);
				return $this->_removeTagsAuxReverse(OjsString::substr($string, 0, -$openBrack), $loc+$openBrack, $tags, $length);
			}
			return $this->_removeTagsAuxReverse(OjsString::substr($string, 0, -1), $loc+1, $tags, $length) . OjsString::substr($string, $backLoc, 1);
		}
	}
	...
	function _reinsertTags($string, &$tags, $reverse = false) {
		...
		for($i = 0; $i < count($tags); $i++) {
			$length = OjsString::strlen($string);
			if ($tags[$i][1] < OjsString::strlen($string)) {
				if ($reverse) {
					if ($tags[$i][1] == 0) { // Cannot use -0 as the start index (its same as +0)
						$string = OjsString::substr_replace($string, $tags[$i][0], $length, 0);
					} else {
						$string = OjsString::substr_replace($string, $tags[$i][0], -$tags[$i][1], 0);
					}
				} else {
					$string = OjsString::substr_replace($string, $tags[$i][0], $tags[$i][1], 0);
				}
	...
	function _closeTags($string, $open = false){
		...
		OjsString::regexp_match_all("#<([a-z]+)( .*)?(?!/)>#iU", $string, $result);
		$openedtags = $result[1];
