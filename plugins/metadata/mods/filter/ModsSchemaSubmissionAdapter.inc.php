...
class ModsSchemaSubmissionAdapter extends MetadataDataObjectAdapter {
	 ...
	 * Constructor
	 */
	function __construct($assocType) {
	...
	function &injectMetadataIntoDataObject(&$modsDescription, &$submission, $replace, $authorClassName) {
		...
		$catalogingLocale = \OjsLocale::getLocaleFrom3LetterIso($catalogingLanguage);
		...
		$submissionLocale = \OjsLocale::get2LetterFrom3LetterIsoLanguage($submissionLanguage);
		if ($submissionLocale) {
	...
	function &extractMetadataFromDataObject(&$submission, $authorMarcrelatorRole = 'aut') {
		...
		$catalogingLocale = \OjsLocale::getPrimaryLocale();
		$catalogingLanguage = \OjsLocale::get3LetterIsoFromLocale($catalogingLocale);
		...
		// Submission language
		$submissionLanguage = \OjsLocale::get3LetterFrom2LetterIsoLanguage($submission->getLanguage());
